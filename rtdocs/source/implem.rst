...
collaboratively generated by the ``explain_opts`` method implemented for
...
on the output directory for the process. The resulting ``explain_opts``
...
result, we define the corresponding ``explain_opts`` method for
