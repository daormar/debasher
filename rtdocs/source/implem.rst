...
In this case, the program only defines one option, ``-s``, for the
...
on the output directory for the process. For this purpose, we define the
``explain_opts`` method for the process, with the following code:
...
result, we define the corresponding ``explain_cmdline_opts`` method for
...
**HINT**: it should be noted that the only goal of the ``explain_opts``
