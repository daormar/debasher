...
In this section we explain how process options are documented for a
DeBasher program. From the set of process options, some of them will
need to be provided via command-line. In particular, command-line
options should be provided when executing the program with
``debasher_exec``. If we execute ``debasher_exec`` with the
``--show-cmdline-opts`` flag, a description of the process options that
should be provided through the command-line is displayed. This
description is collaboratively generated by the ``explain_opts`` method
...
In this case, the program defines one option, ``-s``, for the
``file_writer`` process. In addition to this, the process is also
provided the path of the output file by means of the ``-outf``
option. It is worthy of note that ``-outf`` is not a command line option
that the user needs to provide. Instead, it is defined internally, based
on the output directory for the process.

In order to specify which options are to be provided through the
command-line, we should use another method: the
``identify_cmdline_opts``. This method should specify, from the set of
process options, which ones are command-line options for the program
being defined.

The resulting ``explain_opts`` and ``identify_cmdline_opts`` methods are
defined as follows:
...
will be the output file generated by the ``file_writer`` process. As a
result, we define the corresponding ``explain_opts`` and
``identify_cmdline_opts`` methods for the process as follows:
...
**OBSERVATION**: it should be noted that the only goal of the
``explain_opts`` method is explaining (or documenting) the options of a
...
remains unchanged). In particular, the ``-s`` option is no longer a
command-line option, which is specified by means of the
``identify_cmdline_opts``. The ``explain_opts`` method is unchanged,
since the ``file_writer`` process still receives the ``-s`` option, but
it is no longer provided by the user through the command-line.
