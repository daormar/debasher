Dynamic Fanout Example
^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: bash

    # This program illustrates a fan out stage whose number of parallel
    # worker tasks (w) is decided at workflow execution time, since it
    # comes from a command line option instead of being fixed in the
    # code. "worker_define_opts" reads "-w" from the command line and
    # defines one option list per index in a loop bounded by that
    # value, so the actual number of "worker" tasks only becomes known
    # once "debasher_exec" runs.
    #
    # The fixed upstream stages ("generate", "count", "fragment") hand
    # off to the fan out stage through plain files: "fragment" splits
    # its input into "blk{i}.txt" blocks under its output directory,
    # "dispatch" scans that directory and appends each block name to
    # one of w list files (chosen with "i mod w"), and each "worker"
    # task then reads its own list file line by line.
