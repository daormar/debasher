Dynamic Fanout Example Using FIFOs and Generators
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: bash

    # Same pipeline as debasher_dynamic_fanout_fifos.sh, but the
    # "worker" tasks are created with DeBasher's generator method
    ...
    worker_generate_opts_size()
    {
        ...
        # One worker task per worker
        debasher::read_opt_value_from_line "${cmdline}" "-w"
    }
    ...
    debasher_dynamic_fanout_fifos_gen_program()
    {
