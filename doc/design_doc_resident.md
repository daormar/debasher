...
result. What always fails is a different result with no error (a message
duplicated, lost or altered that nobody reported) and a run that never ends
and reports nothing. The chaos test runs against real `debasher_exec` runs,
not mocks.
