...
- **`CLOSE`**: the last envelope of a writer that has finished for good: it will never write again. It means "finished for good", not "finished successfully": a process that exits with an error sends none and counts as crashed. A halt is not a finish for good, since the node is resumed later, so an ordered halt sends none (decided and done on 2026-09-20, see point 3). The reader hands it to the brain thread in order and then keeps reading, but drops everything that follows it (decided and done on 2026-09-20; the `Supervisor`'s readers deliver it, since a node that closed its channel may be relaunched and must be heard). `closed_ports` is the checkpoint field that lists the input ports whose `CLOSE` the brain thread had processed when the round captured the node state.
...
- **Both nodes of a channel crashed at the same time.** A channel survives the crash of either of its nodes: the other one holds the FIFO open, so what the writer had sent and the reader had not yet read stays in it until the crashed node is back. A node is back, for this purpose, only when it has opened its FIFOs, which a relaunched node does after restoring its checkpoint and replaying its input log: one that is running again but still recovering does not hold them. "At the same time" means that there is a moment at which neither node has the FIFO open: the reader crashes and, before its relaunched incarnation has finished recovering, the writer crashes too, or both are killed together. Then nobody holds the FIFO and what it held is destroyed, at most what a pipe holds (64 KiB). Checked with real `kill -9` on 50 unread lines: all survive if only the writer, or only the reader, dies; none survives if both are dead, whether they died together or half a second apart with no relaunch in between, nor if the writer dies while the relaunched reader is running but still replaying its log (19 of 60 records replayed), where the control without that kill delivers all 50. Whether the loss is repaired depends on where the destroyed messages came from. A relaunched writer restores its latest checkpoint and replays its input log after `processed_upto`, so it sends again, with the same sequence numbers, everything it produced from that point on: the destroyed messages among those reach the reader after all, and it cannot tell (checked with a real relay that had a checkpoint after 10 messages, received 5 more and was killed: the relaunched one sent exactly those 5). What the writer had sent before that checkpoint is inside the node state that it restores and is not sent again: if some of it was still unread in the FIFO, it is lost, and the reader finds the hole in the sequence numbers when the next message arrives (G8). Ways to repair more are listed in Future work (point 7), none designed. Non-adjacent nodes may crash together with no such problem.
...
- **Peer reconnection**: designed, nothing implemented; it is the mechanism behind G3 and G5 across a peer's crash.
...
- **Since the input-log redesign of point 3**: the checkpoint also stores `processed_upto` and `closed_ports` (done);
...
- A halt sends no `CLOSE`. Done on 2026-09-20: `run()` stops its threads with `stop_threads(close=False)`,
  ...
  after its `processed_upto` (measured, see point 3), and a resume would take the node for a finished one. A
...
- **Repairing messages destroyed with a FIFO.** When both nodes of a channel crash at the same time, what
  the FIFO held is destroyed, and only what the writer produces again after its latest checkpoint comes
  back (see the Contract's limits). Ways to widen that, none designed or tried:
