...
########
debasher_builtin_sched_cpus_within_limit()
{
    local cpus=$1
...
########
debasher_builtin_sched_mem_within_limit()
{
    local mem=$1
...
########
debasher_builtin_sched_update_processname_to_idx_info()
{
    local processname=$1
...
########
debasher_builtin_sched_init_process_info()
{
    local cmdline=$1
    local dirname=$2
    ...
    while read process_spec; do
        if debasher::program_process_spec_is_ok "$process_spec"; then
            ...
            # Check cpus value
            debasher_builtin_sched_cpus_within_limit ${cpus} || { echo "Error: number of cpus for process $processname exceeds limit (cpus: ${cpus}, array size: ${array_size}, throttle: ${sched_throttle})" >&2; return 1; }

            # Check mem value
            debasher_builtin_sched_mem_within_limit ${mem} || { echo "Error: amount of memory for process $processname exceeds limit (mem: ${mem}, array size: ${array_size}, throttle: ${sched_throttle})" >&2; return 1; }

            # Register process information
            debasher_builtin_sched_update_processname_to_idx_info ${processname}
...
########
debasher_builtin_sched_revise_reexec_proc_status()
{
...
########
debasher_builtin_sched_release_mem()
{
    local processname=$1
...
########
debasher_builtin_sched_release_cpus()
{
    local processname=$1
...
########
debasher_builtin_sched_get_process_mem()
{
    local processname=$1
...
########
debasher_builtin_sched_get_process_mem_given_num_tasks()
{
    local processname=$1
    local ntasks=$2
...
########
debasher_builtin_sched_reserve_mem()
{
    local processname=$1
    local process_mem=`debasher_builtin_sched_get_process_mem ${processname}`
...
########
debasher_builtin_sched_get_process_cpus()
{
    local processname=$1
...
########
debasher_builtin_sched_get_process_cpus_given_num_tasks()
{
    local processname=$1
    local ntasks=$2
...
########
debasher_builtin_sched_reserve_cpus()
{
    local processname=$1
    local process_cpus=`debasher_builtin_sched_get_process_cpus ${processname}`
...
########
debasher_builtin_sched_get_array_task_status()
{
    local dirname=$1
    local processname=$2
    local task_idx=$3
...
########
debasher_builtin_sched_get_failed_array_task_indices()
{
    local dirname=$1
    local processname=$2
    ...
    for task_idx in `"${SEQ}" 0 ${last_task_idx}`; do
        local task_status=`debasher_builtin_sched_get_array_task_status "${dirname}" $processname $task_idx`
...
########
debasher_builtin_sched_get_finished_array_task_indices()
{
    local dirname=$1
    local processname=$2
    local array_size=${DEBASHER_BUILTIN_SCHED_PROCESS_ARRAY_SIZE[${processname}]}
...
########
debasher_builtin_sched_get_inprogress_array_task_indices()
{
    local dirname=$1
    local processname=$2
    local array_size=${DEBASHER_BUILTIN_SCHED_PROCESS_ARRAY_SIZE[${processname}]}
...
########
debasher_builtin_sched_get_todo_array_task_indices()
{
    local dirname=$1
    local processname=$2
    local array_size=${DEBASHER_BUILTIN_SCHED_PROCESS_ARRAY_SIZE[${processname}]}
...
########
debasher_builtin_sched_get_pending_array_task_indices()
{
    local dirname=$1
    local processname=$2
    local array_size=${DEBASHER_BUILTIN_SCHED_PROCESS_ARRAY_SIZE[${processname}]}
...
########
debasher_builtin_sched_revise_array_mem()
{
    local dirname=$1
    local processname=$2

    local inprogress_tasks=`debasher_builtin_sched_get_inprogress_array_task_indices "${dirname}" ${processname}`
    local num_inprogress_tasks=`debasher::get_num_words_in_string "${inprogress_tasks}"`
    local process_revised_mem=`debasher_builtin_sched_get_process_mem_given_num_tasks ${processname} ${num_inprogress_tasks}`
...
########
debasher_builtin_sched_revise_array_cpus()
{
    local dirname=$1
    local processname=$2
    ...
    local inprogress_tasks=`debasher_builtin_sched_get_inprogress_array_task_indices "${dirname}" ${processname}`
    local num_inprogress_tasks=`debasher::get_num_words_in_string "${inprogress_tasks}"`
    local process_revised_cpus=`debasher_builtin_sched_get_process_cpus_given_num_tasks ${processname} ${num_inprogress_tasks}`
...
########
debasher_builtin_sched_init_curr_comp_resources()
{
    # Iterate over defined processes
    local processname
    for processname in "${!DEBASHER_BUILTIN_SCHED_CURR_PROCESS_STATUS[@]}"; do
        ...
        if [ ${status} = ${DEBASHER_INPROGRESS_PROCESS_STATUS} ]; then
            debasher_builtin_sched_reserve_mem $processname
            debasher_builtin_sched_reserve_cpus $processname
        fi
    done
}

########
debasher_builtin_sched_get_updated_process_status()
{
    local dirname=$1
...
########
debasher_builtin_sched_update_comp_resources()
{
    local dirname=$1
    ...
    for processname in "${!DEBASHER_BUILTIN_SCHED_CURR_PROCESS_STATUS[@]}"; do
        ...
        if [ "${updated_status}" != "" ]; then
            ...
            if [ ${prev_status} = ${DEBASHER_INPROGRESS_PROCESS_STATUS} -a ${updated_status} != ${DEBASHER_INPROGRESS_PROCESS_STATUS} ]; then
                debasher_builtin_sched_release_mem $processname
                debasher_builtin_sched_release_cpus $processname
            fi
            ...
            if [ ${prev_status} != ${DEBASHER_INPROGRESS_PROCESS_STATUS} -a ${updated_status} = ${DEBASHER_INPROGRESS_PROCESS_STATUS} ]; then
                if [ ${process_array_size} -eq 1 ]; then
                    ...
                    # process is an array
                    debasher_builtin_sched_revise_array_mem "${dirname}" $processname
                    debasher_builtin_sched_revise_array_cpus "${dirname}" $processname
                fi
            fi
...
########
debasher_builtin_sched_fix_updated_process_status()
{
...
########
debasher_builtin_sched_get_available_cpus()
{
...
########
debasher_builtin_sched_get_available_mem()
{
...
########
debasher_builtin_sched_check_comp_res()
{
    local processname=$1

    if [ ${DEBASHER_BUILTIN_SCHED_CPUS} -ne ${DEBASHER_BUILTIN_SCHED_UNLIMITED_CPUS} ]; then
        local available_cpus=`debasher_builtin_sched_get_available_cpus`
        process_cpus=`debasher_builtin_sched_get_process_cpus ${processname}`
    ...
    if [ ${DEBASHER_BUILTIN_SCHED_MEM} -ne ${DEBASHER_BUILTIN_SCHED_UNLIMITED_MEM} ]; then
        local available_mem=`debasher_builtin_sched_get_available_mem`
        process_mem=`debasher_builtin_sched_get_process_mem ${processname}`
...
########
debasher_builtin_sched_check_process_deps()
{
    local processname=$1
...
########
debasher_builtin_sched_process_can_be_executed()
{
    local processname=$1

    # Check there are enough computational resources
    debasher_builtin_sched_check_comp_res $processname || return 1
    # Check process dependencies are satisfied
    debasher_builtin_sched_check_process_deps $processname || return 1

    return 0
}

########
debasher_builtin_sched_get_max_num_tasks()
{
    local processname=$1
...
########
debasher_builtin_sched_update_executable_non_array_process()
{
    local processname=$1
    local status=$2
    ...
    if [ ${status} != ${DEBASHER_INPROGRESS_PROCESS_STATUS} -a \
        ...
        if debasher_builtin_sched_process_can_be_executed ${processname}; then
...
########
debasher_builtin_sched_update_executable_array_process()
{
    local processname=$1
    local status=$2
    ...
    if [ ${status} != ${DEBASHER_FINISHED_PROCESS_STATUS} -a \
         ${status} != ${DEBASHER_BUILTIN_SCHED_FAILED_PROCESS_STATUS} ]; then
        if debasher_builtin_sched_process_can_be_executed ${processname}; then
            local max_task_num=`debasher_builtin_sched_get_max_num_tasks ${processname}`
            if [ ${max_task_num} -gt 0 ]; then
                todo_task_indices=`debasher_builtin_sched_get_todo_array_task_indices "${dirname}" ${processname}`
...
########
debasher_builtin_sched_get_executable_processes()
{
    local dirname=$1
    ...
    for processname in "${!DEBASHER_BUILTIN_SCHED_CURR_PROCESS_STATUS[@]}"; do
        ...
        if [ ${array_size} -eq 1 ]; then
            ...
            debasher_builtin_sched_update_executable_non_array_process ${processname} ${status}
        else
            # process is an array
            debasher_builtin_sched_update_executable_array_process ${processname} ${status}
        fi
    done
}

########
debasher_builtin_sched_get_knapsack_cpus_for_process()
{
    local processname=$1
...
########
debasher_builtin_sched_get_knapsack_mem_for_process()
{
    local processname=$1
...
########
debasher_builtin_sched_get_knapsack_name()
{
    local processname=$1
    local task_idx=$2
...
########
debasher_builtin_sched_print_knapsack_spec()
{
    ...
    for processname in "${!BUILTIN_SCHED_EXECUTABLE_PROCESSES[@]}"; do
        ...
        cpus=`debasher_builtin_sched_get_knapsack_cpus_for_process ${processname}`

        # Determine memory requirements
        mem=`debasher_builtin_sched_get_knapsack_mem_for_process ${processname}`

        if [ ${array_size} -eq 1 ]; then
            local knapsack_name=`debasher_builtin_sched_get_knapsack_name ${processname}`
            ...
            for id in ${BUILTIN_SCHED_EXECUTABLE_PROCESSES[${processname}]}; do
                local knapsack_name=`debasher_builtin_sched_get_knapsack_name ${processname} ${id}`
                echo "${knapsack_name} ${DEBASHER_BUILTIN_SCHED_PROCESS_VALUE_FOR_KNAPSACK_SOLVER} ${cpus} ${mem}"
            done
        fi
    done
...
########
debasher_builtin_sched_print_knapsack_sol()
{
    local available_cpus=`debasher_builtin_sched_get_available_cpus`
    local available_mem=`debasher_builtin_sched_get_available_mem`
...
########
debasher_builtin_sched_solve_knapsack()
{
    local dirname=$1
    ...
    "${RM}" -f "${specfile}"
    debasher_builtin_sched_print_knapsack_spec > "${specfile}"

    # Solve knapsack problem
    local knapsack_sol="${dirname}/${DEBASHER_BUILTIN_SCHED_KNAPSACK_SOL_FNAME}"
    debasher_builtin_sched_print_knapsack_sol > "${knapsack_sol}"
...
########
debasher_builtin_sched_count_executable_processes()
{
    echo ${#BUILTIN_SCHED_EXECUTABLE_PROCESSES[@]}
}

########
debasher_builtin_sched_inprogress_processes_pending()
{
    # Iterate over processes
    local processname
    for processname in "${!DEBASHER_BUILTIN_SCHED_CURR_PROCESS_STATUS[@]}"; do
...
########
debasher_builtin_sched_get_debug_process_status_info()
{
...
########
debasher_builtin_sched_get_debug_exec_processes_info()
{
...
########
debasher_builtin_sched_get_debug_sel_processes_info()
{
    local sel_processes
    local knapsack_name
    for knapsack_name in ${DEBASHER_BUILTIN_SCHED_SELECTED_PROCESSES}; do
        sname=`debasher_builtin_sched_extract_process_from_knapsack_name ${knapsack_name}`
        tidx=`debasher_builtin_sched_extract_task_idx_from_knapsack_name ${knapsack_name}`
...
########
debasher_builtin_sched_select_processes_to_be_exec()
{
    local dirname=$1

    # Obtain updated status for processes
    local -A BUILTIN_SCHED_CURR_PROCESS_STATUS_UPDATED
    debasher_builtin_sched_get_updated_process_status "${dirname}"

    # Update computational resources depending on changes
    debasher_builtin_sched_update_comp_resources "${dirname}"
    ...
    debasher_builtin_sched_get_executable_processes "${dirname}"

    if [ ${builtin_sched_debug} -eq 1 ]; then
        local process_status=`debasher_builtin_sched_get_debug_process_status_info`
        ...
        local exec_processes=`debasher_builtin_sched_get_debug_exec_processes_info`
    ...
    if [ ${num_exec_processes} -gt 0 ]; then
        debasher_builtin_sched_solve_knapsack "${dirname}"

        if [ ${builtin_sched_debug} -eq 1 ]; then
            local sel_processes=`debasher_builtin_sched_get_debug_sel_processes_info`
...
########
debasher_builtin_sched_print_pid_to_file()
{
...
########
debasher_builtin_sched_print_script_header()
{
    local fname=$1
    local dirname=$2
    local processname=$3
    ...
    echo "debasher_builtin_sched_print_pid_to_file"
}

########
debasher_builtin_sched_execute_funct_plus_postfunct()
{
    local cmdline=$1
    local dirname=$2
    local processname=$3
...
########
debasher_builtin_sched_print_script_body()
{
    ...
    if [ "${opt_array_size}" -gt 1 ]; then
        ...
        echo "debasher_builtin_sched_execute_funct_plus_postfunct \"\${CMDLINE}\" $(printf '%q' "${dirname}") ${processname} ${opt_array_size} \"\${BUILTIN_ARRAY_TASK_ID}\" > \${builtin_task_log_filename} 2>&1"
    else
        echo "CMDLINE=$(printf '%q' "${cmdline}")"
        local builtin_log_filename=`debasher::get_process_log_filename "${dirname}" ${processname}`
        echo "debasher_builtin_sched_execute_funct_plus_postfunct \"\${CMDLINE}\" $(printf '%q' "${dirname}") ${processname} ${opt_array_size} \"\${BUILTIN_ARRAY_TASK_ID}\" > $(printf '%q' "${builtin_log_filename}") 2>&1"
    fi
}

########
debasher_builtin_sched_print_script_foot()
{
    :
}

########
debasher_builtin_sched_write_env_vars_and_funcs()
{
    local dirname=$1
    ...
    declare -f debasher_builtin_sched_print_pid_to_file
    declare -f debasher_builtin_sched_execute_funct_plus_postfunct
    declare -f debasher::seq_execute_builtin
    declare -f debasher_builtin_sched_get_script_log_filenames
}

########
debasher_builtin_sched_create_script()
{
    # Init variables
    local cmdline=$1
    local dirname=$2
    ...
    debasher_builtin_sched_write_env_vars_and_funcs "${dirname}" | debasher::exclude_readonly_vars >> "${fname}" ; debasher::pipe_fail || return 1

    # Print header
    debasher_builtin_sched_print_script_header "${fname}" "${dirname}" "${processname}" "${opt_array_size}" >> "${fname}" || return 1

    # Print body
    debasher_builtin_sched_print_script_body "${cmdline}" "${dirname}" "${processname}" "${opt_array_size}" >> "${fname}" || return 1

    # Print foot
    debasher_builtin_sched_print_script_foot >> "${fname}" || return 1
...
########
debasher_builtin_sched_wait_until_file_exists()
{
    local pid_file=$1
...
debasher_builtin_sched_launch()
{
    ...
    debasher_builtin_sched_wait_until_file_exists "${pid_file}" ${max_num_iters} || return 1
...
########
debasher_builtin_sched_execute_process()
{
    # Initialize variables
    local cmdline=$1
    local dirname=$2
    ...
    if [ "${launched_tasks}" = "" ]; then
        debasher_builtin_sched_create_script "${cmdline}" "${dirname}" "${processname}" "${opt_array_size}"
    fi

    # Launch script
    local task_array_list=${task_idx}
    debasher_builtin_sched_launch "${dirname}" "${processname}" "${task_idx}" || { echo "Error while launching process!" >&2 ; return 1; }
...
########
debasher_builtin_sched_extract_process_from_knapsack_name()
{
...
########
debasher_builtin_sched_extract_task_idx_from_knapsack_name()
{
    local knapsack_name=$1
...
########
debasher_builtin_sched_exec_processes_and_update_status()
{
    local cmdline=$1
    local dirname=$2
    ...
    for knapsack_name in ${DEBASHER_BUILTIN_SCHED_SELECTED_PROCESSES}; do
        ...
        processname=`debasher_builtin_sched_extract_process_from_knapsack_name "${knapsack_name}"`
        task_idx=`debasher_builtin_sched_extract_task_idx_from_knapsack_name "${knapsack_name}"`

        # Execute process
        debasher_builtin_sched_execute_process "${cmdline}" "${dirname}" "${processname}" "${task_idx}" || return 1
...
########
debasher_builtin_sched_exec_processes()
{
    local cmdline=$1
    local dirname=$2
    ...
    if [ "${DEBASHER_BUILTIN_SCHED_SELECTED_PROCESSES}" != "" ]; then
        local -A BUILTIN_SCHED_CURR_PROCESS_STATUS_UPDATED
        debasher_builtin_sched_exec_processes_and_update_status "${cmdline}" "${dirname}"
    fi
...
########
debasher_builtin_sched_clean_process_files()
{
    ...
    if [ ${array_size} -eq 1 ]; then
        ...
        local pending_tasks=`debasher_builtin_sched_get_pending_array_task_indices "${dirname}" ${processname}`
...
########
debasher_builtin_sched_prepare_files_and_dirs_for_process()
{
    local dirname=$1
    local processname=$2
    local script_filename=`debasher::get_script_filename "${dirname}" ${processname}`
    ...
    if [ "${status}" != "${DEBASHER_FINISHED_PROCESS_STATUS}" -a "${status}" != "${DEBASHER_INPROGRESS_PROCESS_STATUS}" ]; then
        ...
        if [ "${status}" = "${DEBASHER_TODO_PROCESS_STATUS}" ]; then
            ...
            debasher_builtin_sched_clean_process_files "${dirname}" ${processname} || { echo "Error when cleaning log files for process" >&2 ; return 1; }
        fi
...
########
debasher_builtin_sched_prepare_files_and_dirs_for_processes()
{
    local dirname=$1

    local processname
    for processname in "${!DEBASHER_BUILTIN_SCHED_CURR_PROCESS_STATUS[@]}"; do
        debasher_builtin_sched_prepare_files_and_dirs_for_process "${dirname}" $processname
    done
}

########
debasher_builtin_sched_sleep()
{
...
########
debasher_builtin_sched_execute_program_processes()
{
    ...
    debasher_builtin_sched_init_process_info "${cmdline}" "${dirname}" "${procspec_file}" || return 1

    # Revise process status for processes to be reexecuted
    debasher_builtin_sched_revise_reexec_proc_status || return 1

    # Initialize current computational resources
    debasher_builtin_sched_init_curr_comp_resources || return 1

    # Prepare files and directories for processes
    debasher_builtin_sched_prepare_files_and_dirs_for_processes "${dirname}"
    ...
    while [ ${end} -eq 0 ]; do
        ...
        if debasher_builtin_sched_select_processes_to_be_exec "${dirname}"; then
            # Execute processes
            debasher_builtin_sched_exec_processes "${cmdline}" "${dirname}"
            ...
            if debasher_builtin_sched_inprogress_processes_pending; then
...
########
debasher_builtin_sched_get_script_log_filenames()
{
